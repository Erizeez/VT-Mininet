			   ...
			   struct netlink_ext_ack *extack)
{
	...
	if (!exists) {
		ret = tcf_idr_create(tn, index, NULL, a, &act_police_ops, bind,
				     true, 0);
		if (ret) {
	...
	if (P_tab) {
		...
		prate64 = tb[TCA_POLICE_PEAKRATE64] ?
				  nla_get_u64(tb[TCA_POLICE_PEAKRATE64]) :
				  0;
	...
	if (new->peak_present)
		new->tcfp_mtu_ptoks =
			(s64)psched_l2t_ns(&new->peak, new->tcfp_mtu);
	...
	new = rcu_replace_pointer(police->params, new,
...
static int tcf_police_dump(struct sk_buff *skb, struct tc_action *a, int bind,
			   int ref)
{
