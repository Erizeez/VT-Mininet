...
static void do_dilatetimeofday(struct timespec64 *ts)
{
	int dilation;
	...
	if (dilation > 0 && current->virtual_start_nsec > 0) {
		...
		if (dilation < 1000) { // tdf should not be unreasonably large
			...
			s64 dividend = physical_past_nsec -
			...
			s64 divisor =
				div_u64_rem(dividend, dilation, &remainder);
			virtual_past_nsec =
				divisor + current->virtual_past_nsec;
			...
			s64 dilated_now =
				virtual_past_nsec + current->virtual_start_nsec;
			...
			dilated_ts = ns_to_timespec64(dilated_now);
					 ...
					 struct system_device_crosststamp *ts)
{
	...
	partial_history_cycles =
		interp_forward ? total_history_cycles - partial_history_cycles :
				 partial_history_cycles;
	 ...
	 */
	if (discontinuity) {
		corr_real = mul_u64_u32_div(corr_raw, tk->tkr_mono.mult,
					    tk->tkr_raw.mult);
	} else {
 ...
 */
int get_device_system_crosststamp(
	int (*get_time_fn)(ktime_t *device_time,
			   struct system_counterval_t *sys_counterval,
			   void *ctx),
	void *ctx, struct system_time_snapshot *history_begin,
...
int timekeeping_suspend(void)
{
	...
	if (persistent_clock_exists) {
		...
		if (abs(delta_delta.tv_sec) >= 2) {
			...
			timekeeping_suspend_time = timespec64_add(
				timekeeping_suspend_time, delta_delta);
		}
	}
...
static int timekeeping_validate_timex(const struct __kernel_timex *txc)
{
	if (txc->modes & ADJ_ADJTIME) {
		 ...
		 */
		if (txc->modes & ADJ_TICK && (txc->tick < 900000 / USER_HZ ||
