 ...
 */
SYSCALL_DEFINE2(getvirtualtimeofday, struct __kernel_old_timeval __user *, tv,
		struct timezone __user *, tz)
{
	...
	if (likely(tv != NULL)) {
		...
		if (current->virtual_start_time != 0) {
			...
			// printk("[info] [process %d] physical past %ld nanosec from start\n", current->pid, physical_past_time);
			if (dilation > 0) {
				s64 dividend = physical_past_time -
					       current->physical_past_time;
				s64 divisor = div_u64_rem(dividend, dilation,
							  &remainder);
				// dilation is multiplied by 1000 when used as input
				virtual_past_time =
					divisor + current->virtual_past_time;
				now = virtual_past_time +
				      current->virtual_start_time;
				// printk("[info] [process %d] %ld / %d = %ld ... %ld\n", current->pid, dividend, dilation, divisor, remainder);
			} else {
				virtual_past_time =
					(physical_past_time -
					 current->physical_past_time) +
					current->virtual_past_time;
				now = virtual_past_time +
				      current->virtual_start_time;
			}
			// printk("[info] [process %d] virtual past %ld nanosec from start\n", current->pid, virtual_past_time);
			...
			printk("[info] [process %d] getvirtualtimeofday is not using time dilation",
			       current->pid);
		}
...
SYSCALL_DEFINE2(settimedilationfactor, unsigned long, dilation, pid_t, ppid)
{
	if (dilation > 0) {
		...
		if (ppid == 0) // change current's dilation
		{
			...
			ktime_get_real_ts64(&ts);
			current->virtual_start_nsec = timespec64_to_ns(&ts);
			...
			ktime_get_real_ts64(&ts);
			parent->virtual_start_nsec = timespec64_to_ns(&ts);
