...
static void init_virtual_start_time(struct task_struct *task, int dilation)
{
	...
	if (dilation > 0) {
		...
		s64 getnstimeofday_nsec = timespec64_to_ns(&ts);
 ...
 */
SYSCALL_DEFINE2(virtualtimeunshare, unsigned long, unshare_flags, unsigned long,
		dilation)
{
